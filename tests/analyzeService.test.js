...
describe("analyzeService", () => {
	test("should return correct stats for existing tokens", () => {
		...
		const analysis = analyzeService(["the quick"]);
	...
	test("should handle non-existent tokens gracefully", () => {
		const analysis = analyzeService(["non existent token"]);
	...
	test("should return combined stats for multiple tokens", () => {
		...
		Cache.set("the lazy", {
			searchQueriesContainingExactMatch: { "the lazy": 1 },
			searchQueriesContainingFuzzyMatch: {},
			clientDistribution: {
		...
		const analysis = analyzeService(["the quick", "the lazy"]);
